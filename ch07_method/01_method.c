 ...
 *  - 기능을 수행하는 코드 단위
 *  - 반복적인 작업을 피하기 위해 사용
 *  - ex) 매일 코드 10줄 작성 -> 보고서 => 함수 작성 -> 함수 호출 -> 스케줄링 -> 보고서
 *  - main(), printf(), scanf(), ...
 *  - () 붙어있고 카멜케이스를 사용
 * 
 * C언어 함수 사용 방법
 ...
 * 함수 종료
 *  1. return문을 만나면 종료
 *  2. }을 만나서 더 이상 실행할 코드가 없는 경우 종료
 * 
 * 함수 키워드
 *  1. 매개변수: 함수의 입력 값, 여러개 사용 가능(,로 구분)
 *      -매개변수가 없는 함수도 존재 가능
 *  2. return: 기본적으로 함수 종료를 의미
 *      -함수 종료 = 함수를 호출했던 곳으로 다시 돌아간다.
 *      -return 반환값; -> 호출했던 곳으로 돌아가면서 반환값을 가지고 감
 *      -return과 반환값 모두 생략 가능!
 *  3. 매개변수, 데이터반환타입이든 생략시 "void"사용
