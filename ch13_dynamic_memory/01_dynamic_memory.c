 ...
 *   -큰 데이터 구조를 힙 영역에 할당하면 메모리 부족을 해결 가능
 *  3.데이터 구조가 실행 중 동적으로 변하는 경우
 *   -자료구조: Linked List, Tree, Grape, ... 등
 *  4.메모리를 효율적으로 관리할 때
 *   -필요할 때 메모리를 할당하고 해제하여 메모리 낭비를 줄임
 *   -정적배열과 동적배열의 차이
 *   -정적배열은 항상 동일한 크기의 배열을 메모리에 할당
 *   -동적배열은 필요한만큼만 할당하고 추가 및 반환
 *  5.재귀 호출이 많은 경우
 *   -스택 할당 -> 메모리 부족
 *   -동적으로 힙영역에 할당
 *  6.메묄 크기가 유연할 때
 *   -데이터를 추가하거나 삭제할 필요가 있을 때 크기를 조정할 수 있는 메모리를 할당(realloc())
 */
#include <stdio.h>
