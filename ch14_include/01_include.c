 ...
 * 
 *  1단계)전처리
 *   -지시자(#include, #define, ...)에 따라 프로그램에 필요한 소스 중 외부에 있는 소수를 불러옴
 *  ■전처리 된 소스파일
 * 
 *  2단계)컴파일
 *   -CPU가 해석할 수 있는 명령어들로 구성된 기계어로 변환
 *   ■개체파일(abc.o)
 *   -그러나 바로 실행불가(프로그램 운영 체제에 의해 실행되므로 설치된 운영체제가 인식할 수 있게 변경 필요)
 *   + 컴파일 = 컴파일 + 어셈블
 ...
 * 
 *  3단계)링크
 *   -startup code(프로그램 실행 전 필요한 준비작업을)를 결합하는 과정
